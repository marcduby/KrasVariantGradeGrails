...
import org.broadinstitute.variantgrade.heatmap.KrasMatrixParser
import org.broadinstitute.variantgrade.input.SearchInputBean
import org.broadinstitute.variantgrade.result.ProteinResult
import org.broadinstitute.variantgrade.result.kras.KrasProteinResult
...
@Transactional
class KrasMapService {
    // instance variables
    KrasMatrixParser matrixParser = null;
...
//        return this.getMatrixParser().getPlotMap();
    ...
    public ProteinResult getHeatMapReadingFromProtein(int position, String allele, Double prevalence, boolean isStopCodon) throws GradeException {
        // local variables
        Double doubleGradeValue = null;
        String stringGradeValue = null;
        KrasProteinResult result = new KrasProteinResult();
        ...
        if (!isStopCodon) {
            ...
            // get the map amount
            doubleGradeValue = this.getMatrixParser().getDoubleMatrixValueAtPositionAndLetterAndType(position, allele, KrasDataBean.FUNCTIONAL_SCORE, true);
            result.setHeatAmount(doubleGradeValue);

            // get the heat amount standard deviation
            doubleGradeValue = this.getMatrixParser().getDoubleMatrixValueAtPositionAndLetterAndType(position, allele, KrasDataBean.STANDARD_DEVIATION, true);
            result.setHeatAmountStandardDeviation(doubleGradeValue);

            // get the heat amount somatic mutation COSMIC
            doubleGradeValue = this.getMatrixParser().getDoubleMatrixValueAtPositionAndLetterAndType(position, allele, KrasDataBean.COSMIC, true);
            result.setNumberSomaticMutationsCosmic(doubleGradeValue);

            // get the heat amount somatic mutation Genie
            doubleGradeValue = this.getMatrixParser().getDoubleMatrixValueAtPositionAndLetterAndType(position, allele, KrasDataBean.GENIE, true);
            result.setNumberSomaticMutationsGenie(doubleGradeValue);

            // get the heat amount somatic mutation TCGA
            doubleGradeValue = this.getMatrixParser().getDoubleMatrixValueAtPositionAndLetterAndType(position, allele, KrasDataBean.TCGA, true);
            result.setNumberSomaticMutationsTcga(doubleGradeValue);

            // get the heat amount germline mutation ExAC
            doubleGradeValue = this.getMatrixParser().getDoubleMatrixValueAtPositionAndLetterAndType(position, allele, KrasDataBean.EXAC, true);
            result.setNumbergermlineMutationsExac(doubleGradeValue);
...
//        }
     ...
     * @return
     */
    public KrasMatrixParser getMatrixParser() throws GradeException {
        // initialize if not already
        if (this.matrixParser == null) {
            this.matrixParser = KrasMatrixParser.getMatrixParser();
            InputStream inputStream = null;

            // get the file stream and load it to the parser
            inputStream = this.class.classLoader.getResourceAsStream('MutantKRASv2Modified.csv');
...
//            this.matrixParser.setHeatMapStream(MatrixParser.MATRIX_TYPE_MUTATION_PROBABILITY, inputStream);
            ...
            // populate
            this.matrixParser.populate(inputStream);
