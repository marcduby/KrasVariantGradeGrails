package org.broadinstitute.variantgrade.result.kras;
...
public class KrasProteinResult extends ProteinResult {
    ...
    public String getEffect() {
        // local variables
        String effect = KrasProteinResult.EFFECT_NEUTRAL_STRING;

        if (this.getInputPrevalence() != null) {
            if (this.getInputPrevalence() <= 0) {
                effect = KrasProteinResult.EFFECT_DELETERIOUS_STRING;
            } else if (this.getInputPrevalence() >= 1) {
                effect = KrasProteinResult.EFFECT_BENIGN_STRING;
            }
        }

        // set according to logp value
        // only set if not already set as deleterious/benign
        if ((KrasProteinResult.EFFECT_NEUTRAL_STRING.equalsIgnoreCase(effect)) && (this.logP != null)) {
            if (this.logP < 0.0) {
                effect = KrasProteinResult.EFFECT_DELETERIOUS_STRING;
            } else {
                effect = KrasProteinResult.EFFECT_BENIGN_STRING;
